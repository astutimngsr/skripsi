...
function getDataDosenMintaNaikPangkat($where, $isOwner = false){
  $this->db->where($where);
  if ($isOwner) {
    $this->db->where('pegNip !=' , $this->session->userdata('nip'));
  }
...
function getKaryaIlmiahDosenMintaNaikPangkatUntukTim($nip, $where = null){
  ...
  $this->db->join('penilaian_karya_ilmiah','penilaian_karya_ilmiah.id_tim_penilai = '.$this->session->userdata('nip').' AND penilaian_karya_ilmiah.idpublikasi = karya_ilmiah.idpublikasi', 'left');
